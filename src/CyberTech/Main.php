        ...
        switch($command->getName()){
                ...
                if ($args[0] === "trade"){
                    ...
                    if (!isset($args[1])){
                        ...
                        if ($args[1] > $xp){
                            ...
                            $money = (($temp['Money-Exchange']*1)*($args[1]*1));
                            $this->econ->addMoney ( $sender->getName(), $money );
                            $this->db->query("UPDATE stats SET points=points-'$args[1]'");
                            $sender->sendMessage("Success! You Just Traded $args[1] XP for $$money!!!!");
                            return true;
                        }
                    }
                    ...
                    if ($sender instanceof Player){
                        ...
                        $knl = $pp['kill'] + 1;
                        $kxp = $temp['Killer-Levels']['Level'.$knl];
                        $sender->sendMessage("Upgrade Killer Level for: ".$kxp);
                        $dnl = $pp['death'] + 1;
                        $dxp = $temp['Death-Levels']['Level'.$knl];
                        $sender->sendMessage("Upgrade Death Level for: ".$dxp);
                       /* No Miner Levels
                        $knl = $pp['kill'] + 1;
                        $kxp = $temp['Killer-Levels']['Level'.$knl];
                        $sender->sendMessage("Miner Level:".$pp['miner']);*/
                        //Health will only give you extra health once an hour.
                        $hnl = $pp['health'] + 1;
                        $hxp = $temp['Heal-Levels']['Level'.$knl];
                        $sender->sendMessage("Upgrade Health Level for: ".$hxp);
                        $sender->sendMessage("Use /skill upgrade <catagory> /n To Upgrade.");

                        return true;
                    }
                    return true;
                }elseif($args[0] === "upgrade" && isset($args[1])){
                    if ($sender instanceof Player){
                        $xpq = $this->db->query("SELECT * FROM stats WHERE player='$playern'");
                        $xpa = $xpq->fetchArray();
                        $xp = $xpa['points'];
                        if (strtolower($args[1]) === "Killer" ){
                            ...
                            $a = "Death-Levels";
                            $b = "death";
                        }elseif(strtolower($args[1]) === "Health"){
                            $a = "Heal-Levels";
                            $b = "health";
                        }
                        /*elseif(strtolower($args[1]) === "Miner"){
                            $a = "Death-Levels";
                        }*/
                        $levelq = $this->db->query("SELECT * FROM skills WHERE player='$playern'")->fetchArray();
                        $level = ($levelq[$b]*1)+1;
                        $xprequired = $this->skills[$a]["Level".$level];
                        if ($xprequired > $xp){
                            $sender->sendMessage("Uh Oh! You Dont Have Enought XP!".$xprequired." - ".$xp);
                            return true;
                        }else{
                            $this->db->query("UPDATE stats SET points=points-'$xprequired'");
                            $sender->sendMessage("Success! You Are Now A Level".$level." in ".$args[1]."!!!!!");
                            return true;
                        }
                    }
                }else{
                    $sender->sendMessage("Invalid! See /skill help");
                    return true;
                }
                return true;
            case "myskill":
                if ($sender instanceof Player){
                    $playern = $sender->getPlayer()->getName();
                    $pp = $this->db->query("SELECT * FROM skills WHERE player='$playern'")->fetchArray();
            ...
            case "xp":
                if ($sender instanceof Player){
                    ...
                    $xpq = $this->db->query("SELECT * FROM stats WHERE player='$playern'")->fetchArray();
                    $xp = $xpq['points'];
                    $sender->sendMessage("You currently have $xp XP");
                }
                return true;
            case "sheal":
                if ($sender instanceof Player){
                    $playern = $sender->getName();
                    $yml2 = (new Config($this->getServer()->getDataPath() . "/plugins/Skills/" . "cooldowns.yml", Config::YAML ,array()));
                    $yy = $yml2->getAll();
                    ...
                    if($pp < $currentdate){
                        $playern = $sender->getName();
                        $hq = $this->db->query("SELECT * FROM skills WHERE player='$playern'")->fetchArray();
                        $h = $hq['health'];
                        if ($h == 0){
                            $sender->sendMessage("You Havent Leveled Up in Healing!");
                            $sender->sendMessage("Please Level Up and Try Again Later");
                            return true;
                            }
                        $heal = $this->skills['Heal-Reward']["level".$h];
                        $chealth = $sender->getPlayer()->getHealth();
                        if ($chealth >= 50){
                            $sender->sendMessage("You Cant Heal! Your At Full Health!");
                        }else{
                            $th = $heal + $chealth;
                            if ($th > 50){
                                $th = 50;
                            }
                            $sender->getPlayer()->setHealth($th);
                            $sender->sendMessage("You'r Healed!");

                            $yy[$playern] = strtotime("+".$this->skills['Heal-Cool-Down']." Minute ");
                            $yy->setAll($temp);
                            $yy->save();
                        }
                    }else{
                        $remain = $yy[$playern] - $currentdate;
                        $reamin = explode(".",($remain/60));
                        $sender->sendMessage("You Have ".$reamin[0]." Mins Left Before your next heal.");
                    }
                }
                return TRUE;
            default : 
